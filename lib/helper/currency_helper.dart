...
Currency getCurrencyByCode(String code) {
  return Currency.from(
      json: currencies
          .firstWhere((currency) => currency['code'] == code.toUpperCase()));
}

String formatCurrency(double amount, Currency currency) {
  // Format the number with proper decimal digits and separators
  String formattedNumber = amount.abs().toStringAsFixed(currency.decimalDigits);
  ...
  String wholeWithSeparator = '';
  String whole = parts[0];
  for (int i = 0; i < whole.length; i++) {
    if (i > 0 && (whole.length - i) % 3 == 0) {
      wholeWithSeparator += currency.thousandsSeparator;
    }
    wholeWithSeparator += whole[i];
  }

  // Reconstruct number with decimal part if it exists
  String result = parts.length > 1
      ? '$wholeWithSeparator${currency.decimalSeparator}${parts[1]}'
      : wholeWithSeparator;
  ...
  // Add symbol in correct position with proper spacing
